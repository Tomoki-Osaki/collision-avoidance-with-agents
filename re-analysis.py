...
tmp = mf.make_df_trial(df_all, 10, "urgent", 20, 1)
autocorrelation_plot(tmp.dist_from_start); plt.show()
plot_acf(tmp.dist_from_start, lags=len(tmp.dist_from_start)-1); plt.show()


for cond, color in zip(CONDITIONS, mcolors.TABLEAU_COLORS):
    for trial in TRIALS:
        tmp = mf.make_df_trial(df_all, 13, cond, 20, trial)
        if trial == 1:
            plt.plot(tmp.dist_from_start, color=color, label=cond)
        else:
            plt.plot(tmp.dist_from_start, color=color)
plt.legend()
plt.show()
