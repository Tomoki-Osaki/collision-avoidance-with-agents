...
def plot_positions(sim_obj: Simulation, step: int = 0) -> None:
    """
    ...
    for i in range(sim_obj.num_agents):
        pos_px = (sim_obj.all_agents[i]['all_pos'][step]*50)+250
        ...
        if not step == 0:
            tminus1_pos = (sim_obj.all_agents[i]['all_pos'][step-1]*50)+250
