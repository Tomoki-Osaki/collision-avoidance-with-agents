...
cdef class Simulation:     
    ...
    @cython.boundscheck(False) 
    @cython.wraparound(False) 
    cpdef void update_parameters (self):
        """
    ...
    cpdef int[:] find_visible_agents (self, 
        DTYPE_t[:,:] dist_all, 
        int num
    ):
        """
        ...
        near_agents = np.zeros(self.num_agents, dtype=np.int32)
        for i in range(self.num_agents):
            if (dist_all[num][i] != 0.2 
                and 
                dist_all[num][i] < VIEW
                and i != num):
                near_agents[i] = 1
                
        visible_agents = np.zeros(self.num_agents, dtype=np.int32)
        ...
        for i in near_agents:
            ...
            if angle_difference <= self.viewing_angle / 2:
                visible_agents = np.append(visible_agents, i)
        
        return visible_agents
    

    cpdef DTYPE_t[:,:] calc_distance_all_agents (self):
        cdef: # return
            DTYPE_t[:,:] dist_all
        cdef:
            int i, j
    ...
    cpdef DTYPE_t[:] simple_avoidance (self, 
        int num
    ):
        cdef: # return
            DTYPE_t[:] avoid_vec
    ...
    @cython.boundscheck(False) 
    @cython.wraparound(False) 
    cpdef DTYPE_t[:] dynamic_avoidance (self, 
        int num
    ):
        """
        動的回避ベクトルの生成
    ...
    @cython.boundscheck(False) 
    @cython.wraparound(False) 
    cpdef void record_start_and_goal (self, 
        int num
    ):
        """
    ...
    @cython.boundscheck(False) 
    @cython.wraparound(False) 
    cpdef void check_if_goaled (self):
        """
    ...
    cpdef int[:,:] approach_detect (self, 
        DTYPE_t dist
    ): 
        """ 
    ...
    @cython.boundscheck(False) 
    @cython.wraparound(False) 
    cpdef DTYPE_t[:] record_agent_information (self):
        cdef: # return
            DTYPE_t[:] row
        cdef:
            int i
    ...
    @cython.boundscheck(False) 
    @cython.wraparound(False) 
    cpdef int[:] record_approaches (self,
        str approach_dist, 
        int step, 
        list sim_data
    ):
    ...
    @cython.boundscheck(False) 
    @cython.wraparound(False) 
    cpdef DTYPE_t calc_completion_time (self, 
        int num
    ):
        """ 
        ゴールまで到達した際の完了時間を記録
    ...
    @cython.boundscheck(False) 
    @cython.wraparound(False) 
    cpdef DTYPE_t calc_remained_completion_time (self, 
        int num, 
        int step
    ):
        """
