...
# numpyの型ヒントについて、np.ndarray[dtype, ndim]の形で表す。
# 例えば以下の場合、float型のデータを格納した次元数2のarrayとなる。
# np.ndarray[float, 2]

# %% functions to calculate the vectors
def calc_rad (pos2: np.ndarray[float, 1],
              pos1: np.ndarray[float, 1]) -> float: 
    """
...
def rotate_vec (vec: np.ndarray[float, 1],  
                rad: float) -> np.ndarray[float, 1]: 
    """
...
class Simulation:
    def __init__ (self, 
        ...
        for n in range(self.num_agents):
            # グラフ領域の中からランダムに座標を決定
            pos = np.random.uniform(-self.field_size, self.field_size, 2)
            vel = np.random.uniform(-self.field_size, self.field_size, 2)
        ...
        # エージェントにゴールを8ずつ設定
        for i in range(self.num_agents):
            goals = [self.set_goals(self.all_agents2[i]['p'], self.all_agents2[i]['v']) 
                     for _ in range(8)]
    ...
    def set_goals (self, 
                   agent_pos: np.ndarray[float, 1], 
                   agent_vel: np.ndarray[float, 1]) -> np.ndarray[float, 1]: 
        """ 
        ゴールのxy座標の計算
        ...
        while True:
            ...
            # x座標がグラフ領域を超える
            if (agent_pos + agent_vel)[0] < -self.field_size:
                # 超えた時の座標をゴールとする
                goal = agent_pos + agent_vel
                
                # y座標も同時にサイズを超えるかつyが正
                if (goal[1] > self.field_size - 0.1):
                    # ゴールの座標がグラフ領域の角にならないように調整
                    goal[1] -= 0.1
                    
                # y座標も同時にサイズを超えるかつyが負
                elif (goal[1] < -self.field_size + 0.1):
                    # ゴールの座標がグラフ領域の角にならないように調整
                    goal[1] += 0.1
                    
                goal[0] = -self.field_size
                # 端に到達したエージェントを、反対側の端に移動させる
                agent_pos[0] = self.field_size + ((agent_pos + agent_vel)[0] + self.field_size)
                break
                        
            elif (agent_pos + agent_vel)[0] > self.field_size:
                goal = agent_pos + agent_vel
            ...
            # y座標がグラフ領域を超える
            elif (agent_pos + agent_vel)[1] < -self.field_size:
                # 超えた時の座標をゴールとする
                goal = agent_pos + agent_vel
                goal[1] = -self.field_size
                
                agent_pos[1] = self.field_size + ((agent_pos + agent_vel)[1] + self.field_size)
                break
    ...
    def calc_distance_all_agents (self) -> np.ndarray[float, 2]:
        """ 
    ...
    def find_visible_agents (self, 
                             dist_all: np.ndarray[float, 2], 
                             num: int) -> list[int]:
        """
        ...
        # ゴールベクトルの角度を算出する
        to_goal_rad = calc_rad(self.agent_goals[num][self.goal_count[num]], 
                               self.all_agents[num]['p'])
        goal_angle = np.degrees(to_goal_rad)

        for i in near_agents:
            # 近づいたエージェントとの角度を算出
            to_other_rad = calc_rad(self.all_agents[i]['p'], self.all_agents[num]['p'])
            agent_angle = np.degrees(to_other_rad)
    ...
    def simple_avoidance (self, 
                          num: int) -> np.ndarray[float, 1]: 
        """
        単純な回避ベクトルの生成
        ...
        for i in visible_agents:
            ...
            d /= (dist_all[num][i] + 2 * self.agent_size) # 大きさ1のベクトルにする
            d *= self.simple_avoid_vec # 大きさを固定値にする
    ...
    def record_start_and_goal (self, 
                               num: int) -> None:
        """
        ...
        """
        # 前回のゴールが左端にあるとき
        if (self.goal_pos[num][0] == -self.field_size):
            xy, i_xy = 0, 1
            add = 2*self.field_size
        
        # 前回のゴールが右端にあるとき
        elif (self.goal_pos[num][0] == self.field_size):
            xy, i_xy = 0, 1
            add = -2*self.field_size        
        
        # 前回のゴールが下端にあるとき
        elif (self.goal_pos[num][1] == -self.field_size):
            xy, i_xy = 1, 0
            add = 2*self.field_size
    ...
    def check_if_goaled (self) -> None:       
        for i in range(self.num_agents):
            pos_x = (self.all_agents[i]['p'] + self.all_agents[i]['v'])[0]
            pos_y = (self.all_agents[i]['p'] + self.all_agents[i]['v'])[1]
            
            # x座標が左端をこえる
            if pos_x < -self.field_size:
                xy, i_xy = 0, 1
                add = 2*self.field_size 
            
            # x座標が右端をこえる
            elif pos_x > self.field_size:
                xy, i_xy = 0, 1
                add = -2*self.field_size
            
            # y座標が下をこえる
            elif pos_y < -self.field_size:
                xy, i_xy = 1, 0
                add = 2*self.field_size
            ...
            my_posx = self.all_agents[i]['p'][0]
            my_posy = self.all_agents[i]['p'][1]
            ...
            if (my_posx > goal_posx - 0.1 and 
                ...
                # はみ出た時用のゴールに到着
                    ...
                    self.goal_temp[i] = np.zeros(2) # はみ出た時用のゴールを初期化
            ...
            current_pos = (self.all_agents[i]['p'] + self.all_agents[i]['v'])[xy] 
            self.all_agents[i]['p'][xy] = current_pos + add
    ...
    def approach_detect (self, 
                         dist: float) -> np.ndarray[int, 2]: 
        """ 
    ...
    def record_agent_information (self) -> np.ndarray[1]:
        """
        全エージェントの位置と速度、接近を記録
        """
        # 初期の位置と速度を記録
    ...
    def calc_completion_time (self, 
        ...
        step_to_goal = self.goal_step[num] - self.start_step[num] + 1
        dist_to_goal = np.linalg.norm(self.start_pos[num] - self.goal_pos[num])
        completion_time = step_to_goal / dist_to_goal
                         
        return completion_time
    
    
    def calc_remained_completion_time (self, 
                                       num: int, 
                                       step: int) -> float:
        """
        ...
        # ゴールする前に境界をはみ出ている場合
        if not all(self.goal_temp[num] == 0):
            # 左右の境界をはみ出た
    ...
    def return_results_as_df (self) -> pd.core.frame.DataFrame:
        ...
        assert self.current_step == self.num_steps, (
            f"The simulation have not been completed yet. ({self.current_step}/{self.num_steps})"
        )
